...
const LoginPage = () => {
  ...
  const onsubmitHandler =(event) =>{
    ...
    if(currState.toLowerCase() === 'sign up' && !isDataSubmitted){
      setIsDataSubmitted(true)
      return;
    }
    login(currState.toLowerCase() === 'sign up' ? 'signup' : 'login' ,{fullName: FullName,email,password,bio})
  }
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      <div className='flex flex-col items-center gap-3'>
        <img src={assets.logo_icon} alt="Chatify Logo" className='w-[min(30vw,250px)]' />
        <h1 className='text-3xl font-bold text-violet-400 tracking-wide'>Chatify</h1>
      </div>
      ...
      {currState.toLowerCase() === 'sign up' && !isDataSubmitted && (
      ...
      {
        currState.toLowerCase() === 'sign up' && isDataSubmitted && (
          <textarea onChange={(e) =>setBio(e.target.value)} value={bio} rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500' placeholder='Provide a bio'></textarea>
        )
      }
