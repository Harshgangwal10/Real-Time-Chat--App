...
const SideBar = () => {
  const { users, selectedUser, getUsers, unseenMessages, setUnseenMessages, setSelectedUser } = useContext(ChatContext)
    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ''}`}>
      <div className='pb-5'>
        <div className='flex justify-between items-center'>
          <p className="text-3xl font-bold text-violet-400 tracking-wide">Chatify</p>
      ...
      <div className='flex flex-col'>
        {filteredUsers.map((user, index) => (
          <div
            key={index}
            onClick={() => {
              //  toggle open - close if the same user is clicked
